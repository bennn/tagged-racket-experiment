...
(define (try-add-rectangle grid pos height width direction)
  ...
  (define-values [x y] (values (vector-ref pos 0) (vector-ref pos 1)))
  ;bg;(match-define (vector x y) pos)
...
(define (commit-room grid room)
  (for ([pos+cell% (in-list (room-poss->cells room))])
    ;bg;(match-define (cons pos cell%) pos+cell%)
    (define-values [pos cell%] (values (car pos+cell%) (cdr pos+cell%)))
...
(define (list-intersect x* y*)
  (for/list : (Listof A) ([x (in-list x*)] #:when (not (for/or : Boolean ((y (in-list y*))) (equal? x y))))
    x))
    ...
    (cond [(not (= n 0)) ; we got stuck, try again
          ...
          [else ; we did it
           ...
           (for ([r1+r2 (in-list potential-connections)])
             ;bg;(match-define (cons r1 r2) r1+r2)
             (define-values [r1 r2] (values (car r1+r2) (cdr r1+r2)))
             (define common
               ;(set->list
               ;  (set-intersect (list->set (room-extension-points r1))
               ;                 (list->set (room-extension-points r2)))))
                 (list-intersect (room-extension-points r1)
