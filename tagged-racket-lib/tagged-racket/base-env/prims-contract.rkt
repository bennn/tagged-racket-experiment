...
(lazy-require ["../rep/type-rep.rkt" (Error?)]
              ["../types/utils.rkt" (fv)]
              [tagged-racket/private/parse-type (parse-type)])
...
(define-values (require/typed-legacy require/typed -unsafe-require/typed)
 (let ()
  ...
  (define ((r/t-maker legacy unsafe?) stx)
    (unless (or (unbox tagged-context?) (eq? (syntax-local-context) 'module-begin))
      (raise-syntax-error #f "only allowed in a Tagged Racket module" stx))
    (syntax-parse stx
...
;; make-contract-def-rhs/from-tagged : Id Boolean Boolean -> Syntax
(define (make-contract-def-rhs/from-tagged id flat? maker?)
   ...
   (λ ()
     (define type-stx
       (or (cast-table-ref id)
           #f))
     `#s(contract-def ,type-stx ,flat? ,maker? tagged))))
...
(define (core-cast stx)
  (syntax-parse stx
    [(_ v:expr ty:expr)
     ...
     (cond [(not (unbox tagged-context?)) ; no-check, don't check
            #'v]
           [else
            ...
            (define existing-ty-ctc (syntax-local-lift-expression
                                     (make-contract-def-rhs/from-tagged existing-ty-id #f #f)))
               ...
               #,(apply-contract
                  (apply-contract
                   #`(#,(casted-expr-property #'#%expression store-existing-type)
                      v)
                   existing-ty-ctc 'tagged-world 'cast)
                  new-ty-ctc 'cast 'tagged-world))])]))
       ...
       (with-syntax* ([nm #'name.nm]
                     ...
                     (quasisyntax/loc stx
                       (begin
                         ...
                         #,(ignore #'(require/contract pred hidden (or/c struct-predicate-procedure?/c (c-> any/c boolean?)) lib))
