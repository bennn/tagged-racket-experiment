...
1. macros for defining type annotated code.
   ...
   Some of these are provided even in `tagged/racket/base` where the corresponding
  ...
  (let ()
    (define ((mk id) stx)
      (syntax-case stx ()
        [(_ . r)
         #`(let ()
             (local-require (only-in (submod tagged-racket/base-env/prims for-set) #,id))
