...
(define (with-type-helper stx body fvids fvtys exids extys resty expr? ctx)
  (define old-context (unbox tagged-context?))
  (unless (not old-context)
    (tc-error/stx stx "with-type cannot be used in a Tagged Racket module."))
  (set-box! tagged-context? #t)
  (do-standard-inits)
  ...
  (define-values (ex-ctc-ids ex-ctc-defs)
    (type-stxs->ids+defs (syntax->list extys) 'tagged))
  (define-values (region-ctc-ids region-ctc-defs)
    (if expr?
        (type-stxs->ids+defs (values-stx->type-stxs resty) 'tagged)
        (values null null)))
  ...
  (set-box! tagged-context? old-context)
  ;; then clear the new entries from the env ht
  (for ([i (in-syntax fvids)])
    (unregister-type i))
  ;; report errors after setting the tagged-context? flag and unregistering
  ...
  (with-syntax ([(fv.id ...) fvids]
                ...
                [defended-body (maybe-defend #`(#,expanded-body))]
                [(body) (maybe-optimize #'defended-body)]
