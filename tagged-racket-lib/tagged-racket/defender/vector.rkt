...
(require syntax/parse
         ...
         #;(only-in tagged-racket/base-env/prims-contract core-cast)
         ;; logging? fixnum?
         )
...
(define-merged-syntax-class flvector-op
  (flvector-ref^ flvector-set!^ extflvector-ref^ extflvector-set!^))
...
(define-syntax-class vector-opt-expr
  ...
  #:attributes (opt)
  (pattern (#%plain-app ref:vector-ref^ v:opt-expr i:opt-expr)
    #:with opt
    (dynamic-typecheck #'(#%plain-app ref v.opt i.opt) (type-of this-syntax)))
